...
let readMessage = (arrayBuffer, readCallback) => {
...
let dispatchMessage = (messageType, bytes, callback) => {
...
let typeKEEP = (messageType, bytes, keepCallback) => {
...
let typeUTF8 = (messageType, bytes, utf8Callback) => {
...
let type0001 = (messageType, bytes, callback0001) => {
...
let typeARGB = (messageType, bytes, argbCallback) => {
...
let canvasImageData = (canvas, x, y, imageWidth, imageHeight, bytes) => {
...
let makeARGB = (canvas, x, y, callback) => {
  let cb = callback
  try {
    var ctx = canvas.getContext('2d')
    ...
    jsIntTo4Bytes(width, (widthBytes) => {

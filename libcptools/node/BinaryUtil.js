...
var byteArrayToLong = function byteArrayToLong(byteArray) {
  var value = 0;
...
var testLong = function testLong(value) {
  console.log('value', value);
  var baLong = longToByteArray(value);
  console.log('baLong', baLong);
  var valLong = byteArrayToLong(baLong);
...
var bytesFromHex = function bytesFromHex(str, pad) {
...
var jsIntToBytes = function jsIntToBytes(v, f) {
  var c = 0;
  ...
  var testValue = function testValue(v) {
    c++;
    if (c >= 8) {
      jsIntBytes[7 - 7] = 0;
...
var jsIntTo4Bytes = function jsIntTo4Bytes(v, f) {
  if (v < 0) {
    f(null);
    return;
  }
...
var bytesToJsInt = function bytesToJsInt(bytes) {
...
var readMessage = function readMessage(arrayBuffer, readCallback) {
  ...
  var baDataLen = [uint8s[4], uint8s[5], uint8s[6], uint8s[7], uint8s[8], uint8s[9], uint8s[10], uint8s[11]];
  console.log('DATA LEN representation' + baDataLen);

  // Data length value
  var dataLen = bytesToJsInt(baDataLen);
  ...
  dispatchMessage(messageType, bytes, readCallback);
};

var dispatchMessage = function dispatchMessage(messageType, bytes, callback) {
  typeKEEP(messageType, bytes, callback);
  typeUTF8(messageType, bytes, callback);
  type0001(messageType, bytes, callback);
  typeARGB(messageType, bytes, callback);
};

var typeKEEP = function typeKEEP(messageType, bytes, keepCallback) {
  if (messageType[0] === 0x4b && messageType[1] === 0x45 && messageType[2] === 0x45 && messageType[3] === 0x50) {
...
var typeUTF8 = function typeUTF8(messageType, bytes, utf8Callback) {
...
var type0001 = function type0001(messageType, bytes, callback0001) {
...
var typeARGB = function typeARGB(messageType, bytes, argbCallback) {
  if (messageType[0] === 0x41 && messageType[1] === 0x52 && messageType[2] === 0x47 && messageType[3] === 0x42) {
    // Image
    ...
    // ###
    var imageWidth = bytesToJsInt([bytes[0], bytes[1], bytes[2], bytes[3]]);
    var imageHeight = bytesToJsInt([bytes[4], bytes[5], bytes[6], bytes[7]]);
    console.log('===>HEIGHT [0,1,2,3] = ' + bytes[0] + ' ' + bytes[1] + ' ' + bytes[2] + ' ' + bytes[3] + ' ===> ' + imageWidth);
    console.log('===>WIDTH [4,5,6,7] = ' + bytes[4] + ' ' + bytes[5] + ' ' + bytes[6] + ' ' + bytes[7] + ' ===> ' + imageHeight);
    console.log('===>IMAGE [8,9,10,11] = ' + bytes[8] + ' ' + bytes[9] + ' ' + bytes[10] + ' ' + bytes[11]);
    argbCallback({
      type: 'ARGB',
      info: {
...
var canvasImageData = function canvasImageData(canvas, x, y, imageWidth, imageHeight, bytes) {
...
var makeKEEP = function makeKEEP(callback) {
...
var makeUTF8 = function makeUTF8(str, utf8Callback) {
  if (!str || str === '') {
  ...
  try {
    ...
    // byteArray[3] = 0x63 // c
    // byteArray[3] = 0x6d // m
    jsIntToBytes(baDataLen, function (res) {
...
var make0001 = function make0001(callback) {
...
var makeARGB = function makeARGB(canvas, x, y, callback) {
  var cb = callback;
  try {
    var ctx = canvas.getContext('2d');
    ...
    var _jsIntTo4Bytes = undefined.jsIntTo4Bytes;
    var _jsIntToBytes = undefined.jsIntToBytes;
    var _bytesToJsInt = undefined.bytesToJsInt;

    _jsIntTo4Bytes(width, function (widthBytes) {
      // byte array representation of width
      // ===> get height's too
      _jsIntTo4Bytes(height, function (heightBytes) {
        // bytearray representation of height

        console.log('IMAGE widthBytes: ' + widthBytes + ', heightBytes: ' + heightBytes);

        // data length's 8 bytes representation
        _jsIntToBytes(baDataLen, function (res) {
          console.log('**** LEN BYTES ****' + res + ' ==> ' + _bytesToJsInt(res));
...
module.exports.longToByteArray = longToByteArray;
module.exports.byteArrayToLong = byteArrayToLong;

module.exports.testLong = testLong;
module.exports.bytesFromHex = bytesFromHex;

module.exports.jsIntToBytes = jsIntToBytes;
module.exports.jsIntTo4Bytes = jsIntTo4Bytes;
module.exports.bytesToJsInt = bytesToJsInt;

module.exports.readMessage = readMessage;
module.exports.dispatchMessage = dispatchMessage;

module.exports.type0001 = type0001;
module.exports.typeARGB = typeARGB;
module.exports.typeARGB = typeARGB;
module.exports.typeKEEP = typeKEEP;
module.exports.typeUTF8 = typeUTF8;

module.exports.canvasImageData = canvasImageData;

module.exports.make0001 = make0001;
module.exports.makeARGB = makeARGB;
module.exports.makeKEEP = makeKEEP;
module.exports.makeUTF8 = makeUTF8;
